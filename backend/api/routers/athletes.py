...
def list_athletes(
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            # Subquery or group by to get the latest club for the athlete might be complex, 
            # so we just return the club_id. In the current schema, club is on the athlete but they can change.
            # Actually, core.athlete has club_id. We can join core.club to get club_name.
            query = """
                ...
                WHERE 1=1
            """
            count_query = "SELECT COUNT(*) as total FROM core.athlete WHERE 1=1"
            params = []
            
            if search:
                query += " AND a.full_name ILIKE %s"
                count_query += " AND full_name ILIKE %s"
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                SELECT 
                    ...
                    e.stroke, e.distance_m, comp.course_type,
